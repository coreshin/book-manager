    ...
    $('.book-add').click(function(){
        ...
        $.ajax({
            ...
            success: function(json) {
                ...
                $('.new-post').before('<article class="panel-body post" data-id="' + json.id + '"><span class="post-button"><a class="heart hvr-grow" data-id="' + json.id + '" style="color:#848484;"><i class="fa fa-heart"></i></a><a class="book-edit" aria-pressed="true" href="/books/' + json.id + '/edit"><i class="fa fa-pencil-square-o"></i></a><a class="book-delete" aria-pressed="true" data-id="' + json.id + '"><i class="fa fa-times"></i></a></span><section><h3>' + bookTitle + '</h3><small class="sentence text-muted left-dash">' + bookAuthor + '</small></section><section class="review sentence" style="border: none;"><p class="sentence">' + mult(bookRate,'<span class="rate-star">★</span>') + '</p><p class="mb-0">' + bookComment.replace("\n","<br/>") + '</p><p class="text-muted">' + json.date + '</p></section></article>');
